pipeline {
  ...
  stages {
    ...
    stage('Deploy docker'){
      steps {
		     sh '''
                         ssh -i /var/lib/jenkins/.ssh/application.pem -o StrictHostKeyChecking=no ubuntu@ec2-54-169-201-122.ap-southeast-1.compute.amazonaws.com 'bash-s'< ./deploy.sh \latest
            	        '''
      		}
		}		    
